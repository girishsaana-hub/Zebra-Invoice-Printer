 ...
 * 
 * Arabic text: Using ^AZN for TrueType font rendering with proper RTL support
 * Text alignment: Using ^FB (Field Block) for proper alignment
 */
class ZPLGeneratorService {
  private width = 832;  // 4 inch at 203 DPI
  private margin = 15;
  private topPadding = 45;     // ~0.5 cm
  private bottomPadding = 50;  // ~0.6 cm
  ...
  generateInvoiceZPL(invoice: InvoiceWithDetails): string {
    ...
    const labelHeight = this.topPadding + baseHeight + (invoice.lines.length * lineItemHeight) + this.bottomPadding;
    ...
    zpl += '^FH\n';                            // Hex mode ON
    zpl += '^FX Arabic text support enabled\n'; // Comment
    ...
    // ===== INVOICE TITLE =====
    zpl += `^FO0,${y}^A0N,40,40^FB${this.width},1,0,C^FDINVOICE^FS\n`;
    y += 60;
    ...
    // Arabic BOLD - center aligned
    zpl += `^FO${this.margin},${y}^AZN,28,28^FB${this.width - 2 * this.margin},1,0,C^FDالمبيعات^FS\n`;
    ...
    // Arabic headers (second row) - with proper alignment
    y += 24;
    zpl += `^FO${cItem},${y}^AZN,16,16^FB140,1,0,L^FDبند/وصف^FS\n`;
    zpl += `^FO${cUPC},${y}^AZN,12,12^FB100,1,0,C^FDرمز المنتج العالمي^FS\n`;  // CENTER
    ...
    zpl += `^FO${cVAT},${y}^AZN,12,12^FB140,1,0,C^FDمبلغ ضريبة القيمة المضافة^FS\n`;  // CENTER
    ...
    for (const line of invoice.lines) {
      ...
      // Row 3: UPC code and Arabic label (بند/وصف) - CENTER aligned
      y += 24;
      zpl += `^FO${cItem},${y}^A0N,18,18^FD${line.upc_code || ''}^FS\n`;
      zpl += `^FO${cUPC},${y}^AZN,16,16^FB100,1,0,C^FDبند/وصف^FS\n`;
    ...
    zpl += `^FO${cVAT},${y}^AZN,24,24^FB140,1,0,C^FD+${invoice.total_tax.toFixed(2)}^FS\n`;
    ...
    // Header row - English left, Arabic right aligned
    ...
    // Net due rows - English left, Value middle, Arabic right
    const valX = 280;
    const valWidth = 120;
    ...
    // End label
    // Adjust label length to actual content height + bottom padding
    zpl += `^LL${y + this.bottomPadding}\n`;
